...
-- ============================================
-- Policy: Admins and bursars see all transactions in school
CREATE POLICY "Finance staff see school transactions"
ON financial_transactions FOR SELECT
USING (
  school_id = (SELECT school_id FROM users WHERE id = auth.uid())
  AND (SELECT role FROM users WHERE id = auth.uid()) IN ('admin', 'bursar')
);

-- Policy: Students see their own transactions
...
-- Policy: Parents see transactions for linked children
CREATE POLICY "Parents see linked child transactions"
ON financial_transactions FOR SELECT
USING (
  student_id IN (
    SELECT student_id FROM parent_student_links
    WHERE parent_id = (SELECT id FROM users WHERE id = auth.uid())
    AND parent_student_links.school_id = financial_transactions.school_id
  )
);
